...
비공개 Spring 기반 서비스에서 본인이 담당한 WAR 배포, 환경 설정과 경로 처리 범위를 확인했습니다. 동일한 산출물이 환경별 context path와 설정 차이에서도 동작하도록 애플리케이션과 배포 환경의 책임을 나눈 사례입니다.
...
서버 주소, 인증서, 내부 배포 경로, workflow 원문과 실제 인프라 결과는 포함하지 않았습니다. 현재 글은 확인된 담당 범위와 설계 판단을 일반화해 설명하며, 공개 Tomcat 재현 샘플이나 자동화 테스트는 아직 포함하지 않습니다.

...
| `CS-JAVA-06` | [여러 화면에 흩어진 기준값과 사용자 식별 규칙을 한 흐름으로 정합화한 과정](case-studies/business-rule-consistency.md) | `WORK-EDU-01`, `E2`, 권한 있는 비공개 원본에서 본인 귀속 변경 재확인 + 공개 샘플 11개 테스트 + final PR `33276143715` 8개 job PASS + main Pages `33276278894` PASS | `published` | [합성 Spring Boot member-snapshot 샘플](../02_projects/case-study-samples/business-rule-consistency/README.md): canonical session identity, latest-only policy, explicit Mapper boundary, fail-closed errors | 실제 회사 SSO/session·Mapper SQL/운영 DB·전체 정합성은 별도 증거 전 사용 금지 |
...
`CS-AI-01`의 deterministic fixture 성공은 실제 외부 LLM 정확도 주장이 아닙니다. generation, validation, execution과 result correctness를 분리한 service/evaluation pipeline과 PostgreSQL read-only runtime을 검증한 증거로만 사용합니다.
...
- 공개 안전성 검토
- 확인된 결과와 확인하지 못한 한계
- portfolio regression과 main GitHub Pages build/deploy 성공
...
4. 독립 공개 샘플은 capability에 적합한 public repository 또는 `02_projects/case-study-samples/<slug>/`에 구현합니다.
